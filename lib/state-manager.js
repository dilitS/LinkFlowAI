export class StateManager {
  ...
  getSitePreference(hostname) {
    if (!hostname) return {};
    return this.state.sitePreferences?.[hostname] || {};
  }

  async setSitePreference(hostname, preferencePatch) {
    if (!hostname) return;

    const current = this.getSitePreference(hostname);
    const nextPreferences = {
      ...(this.state.sitePreferences || {}),
      [hostname]: {
        ...current,
        ...preferencePatch
      }
    ...
    await this.setState({ sitePreferences: nextPreferences });
  }

  async toggleSitePause(hostname) {
    const current = this.getSitePreference(hostname);
    await this.setSitePreference(hostname, { paused: !current.paused });
  }
