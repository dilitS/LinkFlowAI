...
// Every system instruction we send to the Prompt API is written in English, so
// each session sees English input on top of whatever the user typed.
export const SYSTEM_PROMPT_LANGUAGE = 'en';

/**
 * Build the `expectedInputs.languages` list for a Prompt API session. Chrome
 * requires it to cover *all* languages the session will receive — the English
 * system prompt plus the user's text — otherwise `create()` can reject with
 * NotSupportedError on devices that actually run the model.
 *
 * @param {...(string|undefined|null)} langs
 * @returns {string[]} unique BCP-47 codes, English first
 */
export function expectedInputLanguages(...langs) {
    const out = [];
    for (const lang of [SYSTEM_PROMPT_LANGUAGE, ...langs]) {
        if (typeof lang !== 'string' || !lang || lang === 'auto') continue;
...
export class ChromeAIProvider {
    ...
    async isPromptLanguageSupported(inputLang, outputLang) {
        ...
        try {
            const expectedInputs = [{ type: 'text', languages: expectedInputLanguages(inputLang) }];
            const expectedOutputs = [{ type: 'text', languages: [outputLang || SYSTEM_PROMPT_LANGUAGE] }];
    ...
    async run({ kind, text, sourceLang, targetLang, systemInstruction, prompt, onStream, signal, temperature, onProgress }) {
        let inputLang = sourceLang && sourceLang !== 'auto' ? sourceLang : undefined;
        ...
        if (kind === 'translate') {
            try {
                ...
                if (error?.code === CHROME_AI_ERRORS.PAIR_UNSUPPORTED) {
                    // `auto` carries no language for the Prompt API to declare;
                    // resolve the real one so expectedInputs matches the text.
                    const resolvedInput = inputLang || (await this.detectLanguage(text)) || undefined;
                    return await this.generateText({ systemInstruction, prompt, onStream, signal, temperature, onProgress, inputLang: resolvedInput, outputLang });
                }
                throw error;
            }
        }

        // Correction and prompt generation go straight to the Prompt API, so an
        // unresolved `auto` would leave the user's language undeclared.
        if (!inputLang && text) {
            inputLang = (await this.detectLanguage(text)) || undefined;
        }
